import mp3Available from "../../assets/sounds/Justin Bieber - Available.mp3";
import mp3Billionaire from "../../assets/sounds/Travie McCoy - Billionaire (feat. Bruno Mars).mp3";
import mp3BurbankHouse from "../../assets/sounds/tsubi club - burbank house.mp3";
import mp3CabinFever from "../../assets/sounds/re6ce - cabin fever.mp3";
import mp3ColdGreenEyes from "../../assets/sounds/Station Earth & Roos Denayer - Cold Green Eyes.mp3";
import mp3LoveScenario from "../../assets/sounds/아이콘 - LOVE SCENARIO.mp3";
import mp3MangoLove from "../../assets/sounds/Shawn Wasabi - Mango Love.mp3";
import mp3Maroon from "../../assets/sounds/Zoology - Maroon.mp3";
import mp3Shelter from "../../assets/sounds/Porter Robinson - Shelter (piano).mp3";
import mp3Sunshine from "../../assets/sounds/POWERS - Sunshine.mp3";
import mp3YoungDumbBroke from "../../assets/sounds/Khalid - Young Dumb & Broke.mp3";

export const songs = [
  {
    src: mp3Available,
    artist: "Justin Bieber",
    title: "Available"
  },
  {
    src: mp3Billionaire,
    artist: "Travie McCoy",
    title: "Billionaire (feat. Bruno Mars)"
  },
  {
    src: mp3BurbankHouse,
    artist: "tsubi club",
    title: "burbank house"
  },
  {
    src: mp3CabinFever,
    artist: "re6ce",
    title: "cabin fever"
  },
  {
    src: mp3ColdGreenEyes,
    artist: "Station Earth & Roos Denayer",
    title: "Cold Green Eyes"
  },
  {
    src: mp3LoveScenario,
    artist: "아이콘",
    title: "LOVE SCENARIO"
  },
  {
    src: mp3MangoLove,
    artist: "Shawn Wasabi",
    title: "Mango Love"
  },
  {
    src: mp3Maroon,
    artist: "Zoology",
    title: "Maroon"
  },
  {
    src: mp3Shelter,
    artist: "Porter Robinson",
    title: "Shelter (piano)"
  },
  {
    src: mp3Sunshine,
    artist: "POWERS",
    title: "Sunshine"
  },
  {
    src: mp3YoungDumbBroke,
    artist: "Khalid",
    title: "Young Dumb & Broke"
  }
];
